...
some(Salaries) #  car::some(Salaries)
...
setwd("G:\\Mi unidad\\R_Work\\Examples_R")
read.table("new_salaries.txt", header = TRUE)
...
par(mfrow = c(2,2))
barplot(tapply(Income, state.region, sd), col = "blue", main = "Desviaci�n de ingresos por region")
...
par(mfrow = c(2,2))
barplot(tapply(Income, state.region, median), col = "blue", 
        main = "Mediana de ingresos por estados")

barplot(tapply(Income, state.region, mean), col = "green", 
        main = "Media de ingresos por estados")

barplot(tapply(Income, state.region, Mg), col = "yellow", 
        main = "Media G de ingresos por estados")

barplot(tapply(Income, state.region, Mar), col = "red", 
...
my_packages_1 <- c("boot", "car", "rgl", "vcd", "qcc", "MASS", "munsell", "plot3D", "animation",
