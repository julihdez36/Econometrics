...
# library(wesanderson) Another palette
...
# Third step --------- Descriptive partial summaries -------------------
...
CPS1985[ ,sapply(CPS1985, is.numeric)]
...
scatterplotMatrix(~ wage + education + experience + age, data = CPS1985)
...
qplot(experience, wage, geom = c("point", "smooth"), color = gender, data = CPS1985) # color = factor(gender)
...
lm(CPS1985$wage ~ CPS1985$experience)

summary(lm(CPS1985$wage ~ CPS1985$experience))
...
# Fourth step --------- Bar chart associated with the categories ---------------

table(ethnicity)

opar <- par()            # We make a copy of the current configuration, "par(opar)" restores

# par(mfrow = c(2,2))

par(mfrow = c(2,2), bg = "lavenderblush", col.main = "brown", col.lab = "navy", 
  col.axis = "blue")


barplot(table(ethnicity), col = terrain.colors(12))

barplot(table(region), col = terrain.colors(5))

barplot(table(occupation), col = terrain.colors(7))

barplot(table(sector), col = terrain.colors(3))

par(mfrow = c(1,1))

barplot(table(sector), col = c("yellow", "blue", "red"))

title(main = "Diagrama de barras")


tabla1 <- table(ethnicity); tabla1

100*prop.table(tabla1)

barplot(tabla1, col = "red")
tabla1
...
qplot(x = sector, y = wage, geom = "jitter", alpha =  I(0.3), data = CPS1985)
...
barplot(counts, main = "Stacked Bar Plot", xlab = "Sector", 
  ylab = "Frequency", col = c("pink", "orange", "lightblue", "red", "blue", "green"))


legend(locator(1), c("worker", "technical", "services", "office", "sales", "management"),
  lty = c(1,1), col = c("pink", "orange", "lightblue", "red", "blue", "green"), box.col = 0)
...
barplot(counts, main = "Stacked Bar Plot", xlab = "Sector", 
        ylab = "Frequency", col = 1:6)

legend(locator(1), levels(occupation),
       lty = c(1,1), col = 1:6, box.col = 0)
...
pie(table(occupation), labels = etiquetas, main = "Porcentaje al sector")
pie(table(occupation), labels = etiquetas, col = 1:6, main = "Porcentaje al sector")


pie(table(occupation), col = terrain.colors(12))
pie(table(occupation), col = heat.colors(5))
...
porcentaje <- round(100*prop.table(table(occupation)), 2); porcentaje

etiquetas <- paste(nombre, "\n", " (", round(porcentaje, 2), "%)", sep = "")
etiquetas

pie3D(tabla.labor, labels = etiquetas, explode = 0.1, main = "Diagrama Circular 3D")
...
fan.plot(tabla.labor, max.span = pi, ticks = 180, 
...
tabla.labor <- table(occupation); prop.table(tabla.labor) 

qcc::pareto.chart(tabla.labor) 

attributes(pareto.chart(tabla.labor))
...
plot(gender ~ occupation, data = CPS1985, 
  main = "Grafico de comparacion entre categoria", 
  border = "grey", col = c("pink2", "lightblue"))


plot(gender ~ occupation, data = CPS1985, 
  main = "Grafico de comparacion entre categoria", 
  density = 55, angle = 45, border = "grey3", col = c("pink2", "lightblue"))

table(gender, occupation)

tabla.GO <- table(gender, occupation); tabla.GO

margin.table(tabla.GO)

margin.table(tabla.GO, 1); margin.table(tabla.GO, 2)

prop.table(tabla.GO)
100*prop.table(tabla.GO) |> round(4)
prop.table(tabla.GO, 1) 
prop.table(tabla.GO, 2)
...
medias.S <- tapply(wage, occupation, mean); medias.S
medianas.S <- tapply(wage, occupation, median); medianas.S
...
barplot(sort(medias.S), col = brewer.pal(3, "Set3"), main = "Barra de Medias", 
...
barplot(sort(m.geomet.S), col = brewer.pal(3, "Set1"), main = "Barra de M.Geometrica",
        density = 60, angle = 90, border = FALSE, lwd = 1, las = 2)

barplot(sort(m.armonic.S), col = brewer.pal(4, "Greys"), main = "Barra de M.Armonica", 
...
barplot(sort(sapply(wage_o, psych::winsor.mean)), col = brewer.pal(3, "Set3"), main = "Media Winsor", 
...
df <- data.frame(muestra_1 = sample(wage, 50, replace = TRUE), muestra_2 = sample(wage, 50, replace = TRUE))

df

do.call(cbind,lapply(df, estadisticos))

sapply(df, estadisticos)
...
barchart(wage ~ occupation, data = media.O, col =  brewer.pal(12, "Set3"))

barchart(wage ~ occupation, data = media.OR, groups = region, 
  auto.key = TRUE, par.settings = simpleTheme(col = terrain.colors(2)))

barchart(wage ~ occupation, data = media.ORE, groups = ethnicity, 
         auto.key = TRUE, par.settings = simpleTheme(col = terrain.colors(4)))
...
interaction.plot(occupation, sector, wage, col = "orange", 
  main = "Graficos de comparacion Media G", xlab = "Niveles de ocupacion", fun = Mg)

interaction.plot(occupation, sector, wage, col = "red", 
  main = "Graficos de comparacion Media H", xlab = "Niveles de ocupacion", fun = Mar)
...
plot.design(wage ~ occupation*sector*gender, fun = "Mg", col = "green3", ylim = c(5,13))

plot.design(wage ~ occupation*sector*gender, fun = "Mar", col = "navy", ylim = c(5,13))
...
boxplot(wage ~ occupation, col = brewer.pal(7, "Set3"), 
...
(lim_sw <- quantile(wage, .75) + 1.5*IQR(wage))
(lim_iw <- quantile(wage, .25) - 1.5*IQR(wage))


boxplot(wage, col = "lightblue", main = "Boxplot con Mc")
boxplot.stats(wage)
boxplot.stats(wage)$out

boxplot(wage ~ occupation, notch = T, varwidth = TRUE, col = brewer.pal(7, "Set3"), 
  main = "Boxplot agrupados por categorias", 
  ylab = "Ingresos", xlab = "Ocupacion")

legend(locator(1), c("worker", "technical", "services", "office", "sales",
                     "management"), lty = c(1,1), col = brewer.pal(12, "Set3"), box.col = 0)



boxplot(wage ~ sector*gender, notch = T, varwidth = TRUE, col = brewer.pal(7, "Set3"), 
        main = "Boxplot agrupados por sector y genero", 
        ylab = "Ingresos", xlab = "Ocupacion")
...
salario.w <- CPS1985[occupation == "worker", 1]
salario.t <- CPS1985[occupation == "technical", 1]
salario.s <- CPS1985[occupation == "services", 1]
salario.o <- CPS1985[occupation == "office", 1]
salario.sa <- CPS1985[occupation == "sales", 1]
salario.m <- CPS1985[occupation == "management", 1]


hist(salario.m, freq = FALSE)

RColorBrewer::brewer.pal(7, "RdGy") -> x 
colorRampPalette(x) -> f

hist(salario.m, freq = FALSE, col = f(6))
lines(density(salario.m), col = 2)


library(vioplot)

vioplot(salario.s, salario.m, salario.o, 
  names = c("services", "management", "office"), col = "aquamarine2")
...
vioplot(salario.w, salario.t, salario.s, salario.o, salario.sa, salario.m,
  names = rownames(table(occupation)), col = terrain.colors(6))
...
stripchart(wage ~ occupation, ylab = "Log de Ingresos", vertical = TRUE, 
  log = "y", pch = 21, bg = "navy", col = brewer.pal(7, "Set3"), 
...
# Se diseï¿½a una funcion para el histograma 
...
grubbs_total <- function(x, alpha = 0.05, num_colas = 2){
  if(num_colas == 2){
    ...
    stop("No es un nï¿½mero de colas permitido.")
  }
  t<-c()
  for (i in 1:length(x)) {
